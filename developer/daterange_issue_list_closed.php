...
<div class="col-lg-12">
	...
	$qry = $conn->query("SELECT * FROM issue_log where date_reported >= '".$_SESSION['startdate']."' &&  date_reported <= '".$_SESSION['enddate']."' && status = 'Closed'");
	
}elseif($_SESSION['login_type'] == 3){
$qry = $conn->query("SELECT * FROM issue_log where date_reported >= '".$_SESSION['startdate']."' &&  date_reported <= '".$_SESSION['enddate']."' && status = 'Closed' && (IT_staff_ass = '".$_SESSION['login_staff_id']."' or reassigned_IT_staff = '".$_SESSION['login_staff_id']."') ");	
	
}else{
	$qry = $conn->query("SELECT * FROM issue_log where date_reported >= '".$_SESSION['startdate']."' &&  date_reported <= '".$_SESSION['enddate']."' && staff_id = '".$_SESSION['login_staff_id']."' && status = 'Closed'");
}
