...
if($_SESSION['login_type'] == 1){
	...
	$_SESSION['rowall'] = $conn->query("SELECT * FROM issue_log  ")->num_rows;
...
$_SESSION['rowopen'] = $rowopen = $conn->query("SELECT * FROM issue_log where status = 'Open' ")->num_rows;
		...
		$chartview = $conn->query("SELECT * FROM issue_log where status = 'Open' ")->num_rows;

	$qry = $conn->query("SELECT * FROM issue_log where status = 'Open'");
...
$_SESSION['rowopen'] = $rowopen = $conn->query("SELECT * FROM issue_log where status = 'Open'  && (IT_staff_ass = '".$_SESSION['login_staff_id']."' or reassigned_IT_staff = '".$_SESSION['login_staff_id']."')")->num_rows;
	...
	$chartview = $conn->query("SELECT * FROM issue_log where status = 'Open' && (IT_staff_ass = '".$_SESSION['login_staff_id']."' or reassigned_IT_staff = '".$_SESSION['login_staff_id']."')")->num_rows;

$qry = $conn->query("SELECT * FROM issue_log where status = 'Open' && (IT_staff_ass = '".$_SESSION['login_staff_id']."' or reassigned_IT_staff = '".$_SESSION['login_staff_id']."')");	
...
$_SESSION['rowopen'] = $rowopen = $conn->query("SELECT * FROM issue_log where status = 'Open' && staff_id =  '".$_SESSION['login_staff_id']."' ")->num_rows;
		...
		$chartview = $conn->query("SELECT * FROM issue_log where status = 'Open' && staff_id =  '".$_SESSION['login_staff_id']."'")->num_rows;

	
	
	$qry = $conn->query("SELECT * FROM issue_log where status = 'Open' && staff_id = '".$_SESSION['login_staff_id']."'");
}	
...
<?php echo ucwords  ($row2['firstname'].' '.$row2['othername'].' '.$row2['surname']) ?>	<?php endwhile; ?>
                      ...
                      <?php if($_SESSION['login_type'] == 2): ?>
	                      <div class="dropdown-divider"></div>
		                      <a class="dropdown-item" href="./index.php?page=close_issue&id=<?php echo $row['id'] ?>">Close this Issue</a>
	                      ...
	                      <div class="dropdown-divider"></div>
	                       <a class="dropdown-item" href="./index.php?page=assign_issue&id=<?php echo $row['id'] ?>">Assign to IT Staff</a>
		                      <div class="dropdown-divider"></div>
		                      <a class="dropdown-item" href="./index.php?page=close_issue&id=<?php echo $row['id'] ?>">Close</a>
		                      <div class="dropdown-divider"></div>
