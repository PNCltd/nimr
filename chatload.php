...
$issue = $conn->query("SELECT * FROM pv_table, users2 where pv_table.staff_id = users2.staff_id && pv_table.id ='".$_SESSION['pv_id']."'" );
...
// User	
$cdata = " user_id = '{$meta['staff_ass']},{$_SESSION['login_staff_id']}' ";
$cdata2 = " user_id = '{$_SESSION['login_staff_id']},{$meta['staff_ass']}' ";
$staff = $conn->query("SELECT * FROM  users2 where staff_id  ='".$meta['staff_ass']."'" );
foreach($staff->fetch_array() as $p =>$q){
	$staffd[$p] = $q;
}
		...
		$chkm = $conn->query("SELECT * from messages where  ($cdata or $cdata2) AND convo_id = '".$_SESSION['pv_id']."' ORDER BY date_created DESC");
		if($chkm->num_rows> 0){
